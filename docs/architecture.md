...
- a separate `Nuxt 3` frontend exists under `apps/web` and already consumes auth, catalog, basket, checkout, profiles and orders;
...
1. Decide whether to keep cookie baskets or unify signed-in baskets server-side.
2. Expand frontend coverage from category listing to brand and product detail routes.
